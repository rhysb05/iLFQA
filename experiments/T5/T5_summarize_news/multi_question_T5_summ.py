...
from iTA_T5_summ import Loading_Model
import time
import pandas as pd
import numpy as np
...
for d in np_data:
    start = time.time()
    answer = res.T5_summ_4_concat_para(d[0])
    end = time.time()
...
df.to_excel("/home/bdlabucdenver/data/QA_Results_T5_4_para_concat_summ.xlsx", index = False, header= False)
