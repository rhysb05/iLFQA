...
from iTA_T5 import Loading_Model
import time
import pandas as pd
import numpy as np
...
data_frame = pd.read_excel("/home/bdlabucdenver/data/QA_BERT.xlsx")
np_data = np.array(data_frame)
records_count = 0
final_data = np.array([['Question', 'Answer','Time', 'Bleu', 'Perplex', 'G_answer']])
for d in np_data:
    start = time.time()
    answer = res.get_response_BERT_T5(d[0])
    end = time.time()
...
df.to_excel("/home/bdlabucdenver/data/QA_Results_BERT_T5.xlsx", index = False, header= False)
